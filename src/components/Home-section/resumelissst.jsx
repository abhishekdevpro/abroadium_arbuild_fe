...
const CustomArrow = ({ direction, onClick }) => (
  <div
    className={`absolute top-1/2 transform -translate-y-1/2 z-30 cursor-pointer ${
      direction === "left" ? "left-[460px]" : "right-[460px]"
    } bg-white rounded-full shadow p-2`}
    onClick={onClick}
  >
    <span className="text-xl text-[#0057FF]">
      {direction === "left" ? "❮" : "❯"}
    </span>
  </div>
);
...
const TemplateSlider = () => {
  ...
  return (
    <div className="bg-[#282B8F] py-20 overflow-hidden relative">
      <div className="text-center mb-12">
        <h2 className="text-4xl md:text-[50px] font-bold text-white leading-tight mb-4">
          Explore Our Wide Range Of Trending <br />
          Templates That Recruiters Love To See
        ...
        <Slider {...settings}>
          {templates.map((template, index) => {
            ...
            if (index === centerSlide) {
              styleClass = "scale-105 z-30 shadow-2xl border-2 border-blue-500";
            } else if (index === centerSlide - 1 || index === centerSlide + 1) {
              styleClass = "scale-95 translate-y-4 opacity-80 z-10";
            } else {
              styleClass = "scale-90 translate-y-10 opacity-50 z-0";
            }

            return (
              <div key={template.id} className="px-2">
                <div
                  className={`relative transition-all duration-500 transform bg-white rounded-xl overflow-hidden ${styleClass}`}
                >
                  <img
                    src={template.image}
                    alt={template.name}
                  ...
                  {index === centerSlide && (
                    <Button
                      variant="primary"
                      size="sm"
                      className="absolute bottom-4 left-1/2 -translate-x-1/2 font-medium py-2 px-4 rounded shadow-lg  transition"
                    >
                      Use this template
                    </Button>
                  )}
