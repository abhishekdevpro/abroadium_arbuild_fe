...
{/* Start Building component */}
  ...
  <div className='flex flex-col items-center'>
    <h1 className=' text-4xl font-bold text-center py-6'>Explore Our Wide Range Of Trending Templates That Recruiters Loves To See
    </h1>
    <div className=' flex gap-2'>
    <div className=' text-lg text-left flex flex-col gap-1'>
      <div><i className="fa-solid fa-check border-2 px-1 py-1 rounded-full text-white bg-green-500 "></i> Chosen & Crafted by Our Experts</div>
      <div><i className="fa-solid fa-check border-2 px-1 py-1 rounded-full text-white bg-green-500 "></i> Backed by our Technology</div>
    </div>

    <div className=' text-lg text-left flex flex-col gap-1 '>      
      <div><i className="fa-solid fa-check border-2 px-1 py-1 rounded-full text-white bg-green-500 "></i> Powered by our Robust AI </div>
       <div><i className="fa-solid fa-check border-2 px-1 py-1 rounded-full text-white bg-green-500 "></i> Get it ready in less than 15 minutes</div>
    </div>
    </div>
  </div>
...
<div className='flex flex-col gap-5 px-5 py-10 md:mx-auto md:w-[70%]'>
  <div className='flex flex-col items-center'>
    <h1 className='text-4xl font-bold text-center py-6'>Looking For Impressive Cover Letters?</h1>
    <p className=' text-lg'>Explore from our 25+ premium Cover Letters, Which are exclusively crafted by our Experts.
    </p>
  </div>
  <div className='flex flex-col justify-center gap-5 items-center md:flex-row'>
    {[
      { src: cvimage1, alt: "Assistant Editor Resume Example", title: "Design 1" },
      { src: cvimage3, alt: "Farmer Resume Template", title: "Design 2" },
      { src: cvimage2, alt: "Farmer Resume Template", title: "Design 3" },
      { src: cvimage4, alt: "Farmer Resume Template", title: "Design 4" }
    ].map((template, index) => (
      <div key={index} className='relative group'>
        ...
        <button
        onClick={handleBuildResume}
        className='absolute top-0 left-0 right-0 bottom-0 m-auto w-fit h-fit bg-blue-600 text-white font-bold py-2 px-4 rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-300' id='home_third'>Create Your CV </button>
      </div>
    ))}
  </div>
</div>
